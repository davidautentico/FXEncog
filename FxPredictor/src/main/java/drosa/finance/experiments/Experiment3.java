...
public class Experiment3 {
	...
	private static int doExtractXfromData(
		...
		try {
			...
			for (int i=20;i<data.size()-20;i++){
				...
				numInputs = 9;
				double labelf = data.get(i+0).getOpen5();
				if (Math.abs(diff30)+Math.abs(diff50)>=0.2
						//Math.abs(maxMins.get(i))>=30
						){
	...
	public static void doCalculateWinPer(
		...
		while (iterModel1.hasNext()){
		    ...
		    INDArray features	= tb.getFeatures();
		    INDArray labels 	= tb.getLabels();
		    INDArray predicted	= model.output(features,false);
		    //comparar SIGNO predicho con signo real
		    for (int b=1;b<features.rows();b++){
	            for (int l=1;l<=1;l++){
	            	INDArray fr = features.getRow(b-1);
	            	INDArray lr1 = labels.getRow(b-1);//real-1
	            	INDArray lr = labels.getRow(b);//real
	            	INDArray pr = predicted.getRow(b);//predicho
	            	
	            	double realN_1		= lr1.getDouble(0);
